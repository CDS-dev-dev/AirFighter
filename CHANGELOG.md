...
- ホットスポットを `discovery` / `resource` / `danger` / `event` の4種類へ分化
- 資源、危険、発見、イベントごとに異なる報酬、迎撃、再訪クールダウンを追加
- 主役導線周辺に環境粒子レイヤーを追加し、MAPの奥行きと密度を強化
- MAP切替後に `solid` / `guide` / `decorative` と衝突対象数を確認する衝突信頼監査を追加

### Changed

- ホットスポットの見た目を種類別のセットピースへ変更し、コード的な単一リング表現を軽減
- ランドマークHUDでホットスポット種別と再訪クールダウンを表示
- 衝突しない導線/装飾と、衝突する物理構造の役割分類をより明確化

### Notes

- 今回のリリースは、AAA級との差として挙げた「オープンワールド密度」「アセット品質」「当たり判定と物理の信頼性」を重点的に潰す改善パス
