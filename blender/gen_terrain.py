...
def terrain_h(x, z):
    # ── ベース起伏 ──
    h = 60.0
    h += math.sin(x * 0.00055 + 0.8) * 32
    h += math.sin(z * 0.00070 + 0.3) * 28
    h += math.sin((x - z) * 0.00042 + 1.1) * 20
    h += math.sin((x + z * 0.6) * 0.00028) * 15

    # ── 北部山脈 ──
    mdt = (z + 1400) / 680
    h += max(0, 1 - mdt*mdt) * (260 + math.sin(x*0.0022+0.7)*72 + math.sin(x*0.006)*38)

    # ── 主要峰 ──
    h += gauss2d(x, z,   200, -1820, 340, 360, 390)
    h += gauss2d(x, z,  -720, -1570, 310, 320, 340)
    h += gauss2d(x, z,   980, -1350, 280, 295, 270)
    h += gauss2d(x, z,    60, -1060, 255, 245, 185)

    # ── 中央〜南部丘陵 ──
    h += gauss2d(x, z,  -180,  -640, 900, 520, 130)
    h += gauss2d(x, z,   620,  -520, 440, 400,  95)
    h += gauss2d(x, z,  -720,   480, 520, 440, 115)
    h += gauss2d(x, z,   380,   580, 360, 320,  80)
    h += gauss2d(x, z,  -160,   920, 320, 280,  65)
    ...
    cxW = max(0, cxD - 130)
    h -= math.exp(-(cxW/62)**2) * 162 * cxA
    h += math.exp(-((cxD-205)/65)**2) * 42 * cxA

    # ── 斜行渓谷 ──
    diagT = ((x - z) + 400) / 160
    diagA = (clamp01((x+700)/500) * clamp01((300-x)/500)
           * clamp01((z-100)/300) * clamp01((900-z)/300))
    h -= math.exp(-(diagT**2)) * 95 * diagA

    # ── 河川 ──
    rvX = 120 + math.sin(z*0.0009)*175 + math.sin(z*0.0025+1)*55
    rvD = abs(x - rvX)
    rvA = clamp01((z+1300)/350) * clamp01(1-(z-1400)/350)
    h -= math.exp(-(rvD/105)**2) * 60 * rvA

    # ── 西部断崖 ──
    if x < -1100:
        cliffX = -1650 + math.sin(z*0.0006)*185 + math.sin(z*0.0018)*65
        h -= clamp01(-(x-cliffX)/360) * 260

    # ── 南部湾 ──
    h -= math.exp(-(x/660)**2) * clamp01((z-660)/340) * clamp01(1-(z-1700)/320) * 80

    # ── 南部半島 ──
    h += math.exp(-(x/155)**2) * clamp01((z-860)/260) * clamp01(1-(z-1720)/340) * 86
...
def terrain_color(x, z, y, h_cache, ix, iz, n):
    ...
    gradX = (hR - hL) / (2 * step)
    gradZ = (hU - hD) / (2 * step)
    slope = clamp01(math.hypot(gradX, gradZ) / 1.0)
