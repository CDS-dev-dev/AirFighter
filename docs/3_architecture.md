...
地表の密度は `createUrbanFabric()` が担当する。低中層のポディウム、屋上設備、発光レールを衝突なし装飾として敷き、地面が単なる平面に見えないようにする。太い近未来建築は `createChunkyMegaBlocks()` が生成し、単体の直方体ではなく、箱・多角柱・リング・外骨格フィンを組み合わせた複合体として扱う。
