...
空中道路は `buildSkyway()` / `buildSkyRing()` でチューブ状に生成する。衝突は通常の Box 判定ではなく、`TubeCorridor` / `RingTubeCorridor` の中心線からの距離で判定し、内側半径より内側は飛行可能、外殻厚みだけ接触対象にする。周期的な入口スロットでは外殻判定を無効化し、チューブ内では建物コライダーより航路を優先して、既存構造物で内部が塞がらないようにする。
