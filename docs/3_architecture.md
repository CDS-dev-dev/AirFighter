...
ビジュアル基準は「BALANCED FUTURE CITY」とする。深い青の夜景をベースに、シアン、マゼンタ、アンバーを点ではなく帯・面・入口サインとしてまとめて使う。暗すぎる軍事都市にはせず、地表と水面は濡れた反射が読める程度に明るくする。一方で細かい発光ドットを散らして画面をチカチカさせない。

GPT Image で作った構造物デザインシートは、背景素材ではなく近景構造物の形状言語として使う。実装では画像をそのまま3D化せず、太い都市ゲート、積層ポディウム、六角コア、外骨格リブ、大きな発光帯へ分解して Three.js の実体ジオメトリに翻訳する。遠景は `neo-tokyo-panorama.png` を Three.js の `scene.background` として使い、黒い壁やシリンダー面が飛行空間へ出ないようにする。近距離で飛行機が避ける構造物はコード側で配置する。
...
地表の密度は `createUrbanFabric()` が担当する。低中層のポディウムと控えめな屋上設備を衝突なし装飾として敷き、地面が単なる平面に見えないようにする。ただし主役の飛行ルートを濁らせないため、床面の強い発光レールや意味の読みづらい四角いライン装飾は避ける。中空の読みやすい進行目標は `createFlightCanyonRoute()` の大型ゲートが担う。ゲートの左右柱、上部デッキ、横コアは衝突対象にし、中央の通過空間だけを開ける。`createCombatArenaRoutes()` はゲート間にミサイル回避用の横壁、オーバーパス、旋回目印になる太い複合ブロックを置き、戦闘導線を単なる景観ではなく遮蔽として使えるようにする。ルート周辺の空白感は `createRouteSideDensity()` が低中層の連続街区と太い近未来ブロックを左右に置いて補い、飛行導線のクリアランスを保ったまま都市の谷を作る。太い近未来建築は `createChunkyMegaBlocks()` が生成し、単体の直方体ではなく、箱・多角柱・リング・外骨格フィンを組み合わせた複合体として扱う。

ゲートは単発配置にしない。進行方向に沿ってカーブする一連のルートとして置き、飛び続けるだけで次のゲートを潜れる向きにする。ゲート内の曖昧な横線や床板は置かず、当たり判定のある左右柱、上部デッキ、横コアだけを厚く見せる。中央部が空洞に見えないように、`createCentralCombatDistrict()` で東京駅やランドマークを隠さない範囲に中層の戦闘ブロックを置く。

NEO 東京のドッグファイトでは、プレイヤーと味方は北側の広い進入路にスポーンし、敵はゲート群と戦闘遮蔽物のある都市アリーナ側に配置する。敵・味方の初期高度は通常東京ビルより上、中空ゲートより下に置き、開始直後から平面旋回だけでなく、ゲート、横壁、ビル谷を使ったミサイル回避が起きるようにする。敵ミサイル、味方ミサイル、プレイヤーミサイルはいずれも NEO 東京の衝突対象に当たるため、遮蔽物を使ってロック後の追尾を切れる。
...
トンネル型構造物は「飛べる高度」にだけ置く。高すぎて背景記号になる環状チューブは通常生成しない。代わりに、レインボーブリッジから都市上部へ曲率を持って上昇する `createRainbowBridgeAscentTube()` を主役の立体飛行ギミックにする。外壁は見えるシェルとして描画し、入口と出口だけ `TubeOpening` で明確化する。外壁から自由に出入りできるように見せないため、内部には中心ガイド、壁面レール、間引いたリブを置きつつ、外殻厚みは衝突対象として扱う。チューブ周辺は `isInTubeReserve()` で広めの予約空間にし、ビル、ゲート、橋、一般高架が重ならないようにする。

ランドマークは見た目だけにしない。`getCollisionObjects()` は建物用コライダーだけでなくランドマークの `Group` も返すため、プレイヤー衝突側では `Box3.setFromObject()` で `Mesh` / `Group` のどちらも扱う。スカイツリー、東京タワー、東京駅、橋脚のような大きな構造物は、見た目があるなら通り抜けられないことを原則にする。東京駅のような実在ランドマークを隠す大きな上部板や、当たり判定の有無が読めない装飾板は置かない。
