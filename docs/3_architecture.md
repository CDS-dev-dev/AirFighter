...
主役となる空中道路だけを `buildSkyway()` でチューブ状に生成する。衝突は通常の Box 判定ではなく、`TubeCorridor` の中心線からの距離で判定し、内側半径より内側は飛行可能、外殻厚みだけ接触対象にする。周期的な入口スロットでは外殻判定を無効化し、チューブ内では建物コライダーより航路を優先して、既存構造物で内部が塞がらないようにする。上層リング、一般高架、橋の接続路は `buildOpenSkyway()` / `buildSkyRing()` の細い非衝突ガイドとして扱い、都市全体をチューブで埋めない。
