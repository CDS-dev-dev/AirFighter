...
主役となる空中道路だけを `buildSkyway()` でチューブ状に生成する。山手線ウェイポイントを使った環状チューブを本線にし、衝突は通常の Box 判定ではなく、`TubeCorridor` の中心線からの距離で判定する。内側半径より内側は飛行可能、外殻厚みだけ接触対象にする。本線は基本的に閉じ、主要ノードから伸びる斜めサイドランプの接続口だけを `TubeOpening` で開ける。チューブ内では建物コライダーより航路を優先して、既存構造物で内部が塞がらないようにする。上層リング、一般高架、橋の接続路は `buildOpenSkyway()` / `buildSkyRing()` の細い非衝突ガイドとして扱い、都市全体をチューブで埋めない。
