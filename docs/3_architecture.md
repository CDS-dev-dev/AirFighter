...
主役となる空中道路だけをチューブ状に生成する。山手線ウェイポイントを使った環状チューブは `CatmullRomCurve3` と `TubeGeometry` で滑らかな曲線として描画し、衝突は細かい `TubeCorridor` に分割して中心線からの距離で判定する。内側半径より内側は飛行可能、外殻厚みだけ接触対象にする。本線は基本的に閉じ、主要ノードから伸びる斜めサイドランプの接続口だけを `TubeOpening` で大きく開ける。描画時も `TubeOpening` 周辺の外殻とリブを抜くため、見た目の入口と衝突判定の入口が一致する。チューブ周辺は `isInTubeReserve()` で予約空間にし、ビル、リング、アーチ、一般高架が本線に重ならないようにする。上層リング、一般高架、橋の接続路は `buildOpenSkyway()` / `buildSkyRing()` の細い非衝突ガイドとして扱い、都市全体をチューブで埋めない。
