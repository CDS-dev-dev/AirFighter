...
### 概要
NEO TOKYOマップは2087年の東京を舞台にした垂直都市。実在する東京のランドマークを保存しながら、空中デッキ、展望リング、上層交通、発光導線を増築していった未来都市として描く。
...
NEO TOKYO ではランドマーク同士をつなぐ空中回廊を主要な飛行体験として扱う。東京駅、東京タワー、レインボーブリッジ、お台場、スカイツリー、浅草寺、新宿、渋谷を結ぶルート上には汎用ビルを置かず、薄い発光レーン、左右レール、通過ゲートで「ここを飛ぶと気持ちいい」導線を示す。

半透明の発光ゲート、アーチ、リング、水面、ホログラムは視覚演出であり、衝突対象にはしない。プレイヤーが「透けているのに通れない」と誤解しないよう、衝突するものは実体のあるビル・塔・ランドマークに限定する。

### 地区構成
- **ランドマーク保護ゾーン**: 東京記号を読ませるため、汎用巨大ビル・巨大支柱を置かない
- **飛行回廊ゾーン**: 中高層ビルや発光レールで、抜け道と速度感を作る
- **外周メガストラクチャー**: 巨大支柱・リング・アーチを配置し、遠景とスケール感を作る
- **湾岸/河川ゾーン**: 地形を水面より下げ、ネオングリッドを水域に重ねない

### 配置ルール
- ランドマーク周辺には保護半径を設け、実在コアとNEO増築がビルで隠れないようにする
- 巨大ビルはランドマークの真横ではなく、外周・背景・飛行回廊の壁として配置する
- 水面は `WATER_LEVEL` に統一し、水域の地形を沈めることで z-fighting を防ぐ
- 山手線・橋・高速道路は「東京らしい導線」として、飛行中に追える太さと高さを優先する
